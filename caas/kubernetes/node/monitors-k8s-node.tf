...
resource "datadog_monitor" "unregister_net_device" {
  ...
  query = <<EOQ
    events(\"sources:kubernetes ${module.filter-tags.event_alert} UnregisterNetDevice\").rollup(\"count\").last(\"${var.unregister_net_device_timeframe}\") > ${var.unregister_net_device_threshold_critical}
EOQ
